...
COMMIT_USERNAME=${COMMIT_USERNAME:-"ZEISS CI"}
COMMIT_EMAIL=${COMMIT_EMAIL:-"no-reply@zeiss.com"}
...
HELM_CHARTS_URL=${HELM_CHARTS_URL:-"https://zeiss.github.io/charts"}
...
upload() {
  ...
  echo "${CT_CHART_DIRS}" | tr "," "\n" | while read -r chartDir; do
    ...
    if [[ -f "${chartDir}/index.yaml" ]]; then
      ...
      mv -f "${CHARTS_TMP_DIR}/${chartDir}"/*.tgz "${chartDir}"
      helm repo index "${chartDir}" --url "${HELM_CHARTS_URL}/${chartDir}"
    fi
